...
var kResultLayerName = "Strata Foto 3D Result";
	...
	"dialog { \
		...
		ui: Panel { \
			...
			gDoc: Group { \
				...
				label: StaticText { alignment: 'right', text: 'Document:' }, \
				docList: DropDownList { alignment: 'left', preferredSize: [300, 20] }, \
			}, \
			gDocName: Group { \
				orientation: 'row', alignment:'right', \
				label: StaticText { alignment: 'right', text: 'Name:' }, \
				name: EditText { alignment: 'left', text: '', preferredSize: [300, 20] }, \
			} \
		} \
		gButtons: Group { \
			orientation: 'column', alignment: 'right', \
...
var docList = win.ui.gDoc.docList;
var docName = win.ui.gDocName;
var choice = -1;
...
{
	choice = 0;
	while (docList.selection != docList.items[choice])
	{
		choice++;
	}

	if (choice >= app.documents.length)
	{
		docName.enabled = true;
		docName.name.active = true;
			// On Windows this is resulting in selected text scrolled off to the left.
			// But now that we leave it empty let's go for it.
	}
	else
	{
		docName.active = false;
		docName.enabled = false;
	}
}


function OnClickOkay (message)
{
	OnChange( message );

	docName = docName.name.text;

	win.close();
}


function OnClickCancel (message)
{
	choice = -1;
...
if (app.documents.length > 0)
{
	...
	{
		var nextDoc = app.documents[ docIndex ];
		if (nextDoc == app.activeDocument)
		{
			choice = docIndex;
		}
		docList.add("item", nextDoc.name);
	}
	docList.add("separator", "");
}
docList.add("item", kNewDocumentItem);
if (choice >= 0)
{
	docList.items[ choice ].selected = true;
}
else
{
	docList.items[ 0 ].selected = true;
}
docList.onChange = OnChange;
win.gButtons.okBtn.onClick = OnClickOkay;
win.gButtons.cancelBtn.onClick = OnClickCancel;
...
if (choice >= 0)
{
	...
	while (docName == "")
	{
		docName = kNewDocumentNamePrefix + nameIndex++;

		for (var docIndex = 0; docIndex < app.documents.length; docIndex++)
		{
			var nextDoc = app.documents[ docIndex ];
			if (nextDoc.name == docName)
			{
				docName = "";
				break;
			}
